/**
 * @overview network module of JJmatch framework
 * @author hewg
 * @createDate 2016-07-13
 * @lastModifiedDate 2016-12-13
 * @lastModifiedBy hewg
 * @version 0.0.2
 */
let is_timeout = false, //超时标识
  timer = null; //计时器
...
const set_data = args => {
  ...
  if (args && args.data) {
    ...
    if (args.type === "get" || args.data_type === "jsonp") {
      args.url +=
        args.url.indexOf("?") > -1 ? "&" + args.data : "?" + args.data;
    }
  }
};
...
const create_xhr = args => {
  ...
  if (args.xhrFields && typeof args.xhrFields == "object") {
    for (i in args.xhrFields) {
      xhr[i] = args.xhrFields[i];
    }
  }
...
export default class Network {
   ...
   * @param {Object} xhrFields 一个具有多个"字段名称-字段值"对的对象，用于对本地XHR对象进行设置
   ...
   * @param {String} dataType 请求的类型 ，可选'json/jsonp'
   */
  static post(url, data, success, dataType) {
    if (is_function(data)) {
      if (typeof success == "string") {
        dataType = success;
      }
